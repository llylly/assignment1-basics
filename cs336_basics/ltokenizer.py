    ...
    **kwargs,
    ...
    with open(input_path, "rb") as f:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            f.seek(start)
            chunk_list.append(f.read(end - start).decode("utf-8", errors="ignore"))
        
        with Pool(num_processes) as p:
            results = p.map(__pre_tokenization, [(chunk, special_tokens) for chunk in chunk_list])
    ...
    for item in pre_toks:
        item[2] = [vocab_reversed[item[0][i:i+1]] for i in range(len(item[0]))] # init the tokenized doc
    merges = list()
    ...
    while len(vocab) < vocab_size - len(special_tokens):
                ...
                for i, tok in enumerate(new_toklist):
                    if tok == len(vocab) - 1: # new token added here
                        for j in [0,1]: # add new bpes, before and after
                            ...
                            new_bpes[(new_bpe_a, new_bpe_b)] = [old_freq + pre_toks[doc_index][1], old_appearing + [doc_index]]
        ...
        for k in new_bpes:
            v = new_bpes[k]
            new_token_pair = TokenPair([k[0], k[1]], v[0], v[1])
