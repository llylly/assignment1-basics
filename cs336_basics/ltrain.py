...
if __name__ == '__main__':
    ...
    # wandb
    wandb.init(project=('LLLM/' + config.save_path).replace('/', '|'), config=asdict(config) | stats, dir=os.path.join(config.save_path, 'wandb_logs'))
    ...
    for now_step in tqdm(range(start_iter, config.trainer.tot_steps), desc='training'):
        ...
        train_info = {'train/loss': loss.item(),
                    'train/lr': now_lr,
        ...
        if val_dataset is not None and now_step % config.val_step == 0:
